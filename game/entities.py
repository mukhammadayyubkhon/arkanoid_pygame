...
class Bonus:
    """ Power-up that falls from destroyed bricks. """
    
    def __init__(self, x: int, y: int, bonus_type: str) -> None:
        self.type = bonus_type
        self.color = cfg.BONUS_TYPES[bonus_type]['color']
        self.icon = cfg.BONUS_TYPES[bonus_type]['icon']
        self.rect = pygame.Rect(
            x - cfg.BONUS_WIDTH // 2,
            y - cfg.BONUS_HEIGHT // 2,
            cfg.BONUS_WIDTH,
            cfg.BONUS_HEIGHT
        )
        self.speed = cfg.BONUS_SPEED
        self.lifetime = cfg.BONUS_LIFETIME
        
    def update(self) -> bool:
        """ Moves bonus down. Returns False if out of screen or expired. """
        self.rect.y += self.speed
        self.lifetime -= 1
        return self.rect.top < cfg.HEIGHT and self.lifetime > 0
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        text = font.render(self.icon, True, cfg.WHITE)
